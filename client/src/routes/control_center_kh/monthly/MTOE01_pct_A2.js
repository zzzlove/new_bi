...
function RenderComponent({data}) {
    ...
    const Chart1Defs = createG2((chart, data) => {
        ...
        chart.intervalStack().position('t_run*count_pct').color('control_element').tooltip('control_element*count_pct*amount');
...
query MyQuery($control_center_id: [Int], $type: [String]) {
  ...
  dataSource: control_center_actual(type: $type, record_type:"monthly", data_set: "level_1",control_center_id: $control_center_id, control_element: "A2", order: ["t_run"]) {
    id
    control_center_id
    t_run
    type
