...
const Chart1Defs = createG2((chart, data) => {
  ...
  chart.intervalStack().position('t_run*amount_compared').color('control_element')
    .label('amount_compared',{offset:5,
      label: {fill: '#676767'},
...
query MyQuery($control_center_id: [Int]) {
  chart_title_one(code: "KH_MTIT02_2"){show_title}
  title: control_center_actual_one(type:["city_cus","line_cus","service_cus","region_cus","linegr_cus"], record_type:"monthly", data_set: "byCusHis", control_center_id: $control_center_id){control_center}
  dataSource: control_center_actual(type:["city_cus","line_cus","service_cus","region_cus","linegr_cus"], record_type:"monthly", data_set: "byCusHis", control_center_id: $control_center_id, order: ["t_run"]) {
    id
    control_center_id
    t_run
    type
