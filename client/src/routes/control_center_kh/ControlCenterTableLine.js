...
class RenderComponent extends React.Component {
 ...
 render() {
  ...
  const searchGroupProps = {
    ...
    size: 'large',
    select: true,
    selectOptions: [{ value: 'name', name: '线路名称' }],
    selectProps: {
      defaultValue: field,
    },
    onSearch: (value) => {
   ...
   const columns = [{
    ...
    render: (text, record, index) => (<span>
      ...
      <Button onClick={() => openNewLink(`control_center_kh_line?id=${record.id}`)}>月经营分析 <Icon style={{marginLeft:'1px', padding:'1px', fontSize: 12}} type="pie-chart"/></Button> {'  '}
     </span>),
    }, {
    title: '线路名称(往返)',
    dataIndex: 'name',
...
query MyQuery {
  dataSource: control_center(type: "line", order: ["xorder"]) {
    id
		name
    owner
    cat_1st 
