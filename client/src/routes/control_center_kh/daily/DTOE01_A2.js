...
function RenderComponent({data}) {
    ...
    const Chart1Defs = createG2((chart, data) => {
        ...
        chart.intervalStack().position('t_time*count').color('control_element').tooltip('control_element*count*amount');
...
query MyQuery($control_center_id: [Int], $type: [String]) {
  chart_title_one(code: "KH_DTOE01"){show_title}
  ...
  dataSource: control_center_actual(type: $type, record_type:"daily", data_set: "level_1",control_center_id: $control_center_id, control_element: "A2", order: ["t_run"]) {
    id
    control_center_id
    t_time
    type
