...
//      .shape('control_element', ['rect','hollowRect'])
        ...
        chart.interval(['dodge', 'stack']).position('t_run*count_distinct')
           .color('control_element*cat_1st', function(control_element, cat_1st) { return colorMap[cat_1st]; })
           .tooltip('cat_1st*count_distinct')
           .size(20)
          .label('count_distinct',{offset: 5,label: {fill: '#676767'}});
    ...
    const Chart2Defs = createG2(chart => {
      ...
      // 绘制内部的饼图
      chart.intervalStack()
        .position(Stat.summary.percent('count_distinct'))
    ...
    const Chart3DefsA = createG2((chart, data) => {

        var colDefs3 = {
          control_element: {alias: "签约", values: ['签约', '非签约']},
          cat_1st: {alias: "签约及活跃", values: ['签约-稳定客户', '签约-保有客户', '签约-新客户', '签约-活跃下降', '签约-流失']},
          t_run: {alias: "月份"},
          count_distinct: {alias: "客户数"},
          amount: {alias: "收入(单位: 万元)", formatter: function (amount) {
          return (amount / 10000).toFixed(0);},
        ...
        chart.source(data, colDefs3);

        chart.axis('t_run', {title: false });

        chart.legend('cat_1st', {position: 'top'} );

        chart.intervalStack().position('t_run*amount')
          .color('cat_1st', function(value) { return colorMap[value]; })
    ...
    const Chart3DefsB = createG2((chart, data) => {

        var colDefs3 = {
          control_element: {alias: "签约", values: ['签约', '非签约']},
          cat_1st: {alias: "签约及活跃", values: ['非签约-稳定客户', '非签约-保有客户', '非签约-新客户', '非签约-活跃下降', '非签约-流失']},
          t_run: {alias: "月份"},
          count_distinct: {alias: "客户数"},
          amount: {alias: "收入(单位: 万元)", formatter: function (amount) {
          return (amount / 10000).toFixed(0);},
    ...
    const Chart4DefsA = createG2((chart, data) => {
        ...
        chart.source(data, colDefs4);

        chart.axis('t_run', {title: false });

        chart.legend('cat_1st', {position: 'top'} );

        chart.intervalStack().position('t_run*count_distinct').color('cat_1st', function(value) { return colorMap[value]; })
          .label('count_distinct',{offset: 5,label: {fill: '#676767'}});
    ...
    const Chart4DefsB = createG2((chart, data) => {
        ...
        chart.intervalStack().position('t_run*count_distinct')
          .color('cat_1st', function(value) { return colorMap[value]; })
          .label('count_distinct',{offset: 5,label: {fill: '#676767'}});
      ...
      let dataRender = t_run_selected=='整体合计'?
          _.sortBy(_.filter(dataSource, {cat_2nd: '1'}), 'cat_1st')
        : _.sortBy(_.filter(dataSource, {t_run: t_run_selected, cat_2nd: '1'}), 'cat_1st');


      return (
        <div>
        <Card>
        <h2>{title.control_center}{chart_title_one.show_title}{'的客户数统计, 及'}{t_run_selected}{'百分比分布'}</h2>
        <Row gutter={24} >
        ...
        <Col lg={15} md={24}>
            <Chart1Defs
              data={_.filter(dataSource, {cat_2nd: '1'})}
              forceFit={true}
              width={500}
              height={450}
        ...
        <Col lg={12} md={24}>
            <h2>{'签约客户流失及对应的收入影响'}</h2>
            <Chart3DefsA
              data={_.filter(dataSource, {control_element: '签约'})}
              forceFit={true}
              width={500}
              height={450}
        ...
        <Col lg={12} md={24}>
            <h2>{'非签约客户流失及对应的收入影响'}</h2>
            <Chart3DefsB
              data={_.filter(dataSource, {control_element: '非签约'})}
              forceFit={true}
              width={500}
              height={450}
              plotCfg={{ margin: [45, 30, 50, 80],  }}/>
...
query MyQuery($control_center_id: [Int]) {
  chart_title_one(code: "KH_M2LP02"){show_title}
  title: control_center_actual_one(data_set: "byCusCntrHis", control_center_id: $control_center_id){control_center}
  dataSource: control_center_actual(data_set: "byCusCntrHis", control_center_id: $control_center_id, order: ["t_run"]) {
    id
    t_run
    control_center_id
    control_element
