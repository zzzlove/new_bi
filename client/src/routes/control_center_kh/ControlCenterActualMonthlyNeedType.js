...
class RenderComponent extends React.Component {
 ...
 render() {
  ...
  const { loading, error, dataSource, dataOptionsd003, dataOptiong005 } = this.props.data;
  ...
  const doExport = function(){
    var excelId
    if(type == 'service_inc' ){
     excelId = 'controlCenterActualMonthlyServiceInc';
    }else if(type == 'line_inc'){
      excelId = 'controlCenterActualMonthlyLineInc';
    }else if(type == 'city_inc'){
      excelId = 'controlCenterActualMonthlyCityInc';
    }
...
query MyQuery($control_center_id: [Int], $type: [String], $t_time: [Float]) {
  dataSource: control_center_actual(control_center_id: $control_center_id, t_time: $t_time, type: $type, record_type:"monthly", order: ["t_run"]) {
    id
    t_run
    type
    t_time
 ...
 dataOptiong005: field_option(option_code: "g005"){value, text}
}
`;
