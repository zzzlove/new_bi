...
const Chart1Defs = createG2((chart, data) => {
    ...
    chart.intervalStack().position('t_run*count').color('control_element').tooltip('control_element*count*amount')
      .label('count',{offset: 5,label: {fill: '#4FAAEB'}});
...
query MyQuery($control_center_id: [Int]) {
  chart_title_one(code: "KH_WTOE01"){show_title}
  title: control_center_actual_one(type:["city_inc","line_inc","service_inc","region_inc"],record_type:"weekly", data_set: "level_1",control_center_id: $control_center_id){control_center}
  dataSource: control_center_actual(type:["city_inc","line_inc","service_inc","region_inc"],record_type:"weekly", data_set: "level_1",control_center_id: $control_center_id, order: ["t_run"]) {
    id
    control_center_id
    t_run
    control_element
