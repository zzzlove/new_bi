...
function RenderComponent({data}) {
    ...
    const Chart1Defs = createG2((chart, data) => {
        ...
        {
        control_element: {alias: "运作方式",  values: ["A2"]},
        amount: {alias: "产值(单位: 万元)", formatter: function (value) {
        return (value / 10000).toFixed(0);},
        },
        count: {alias: "运单数"},
        ...
        chart.intervalStack().position('t_run*amount').color('control_element').tooltip('control_element*count*amount*pct')
...
query MyQuery($control_center_id: [Int], $type: [String]) {
  chart_title_one(code: "KH_WTIE01"){show_title}
  title: control_center_actual_one(type: $type, record_type:"weekly", data_set: "level_1",control_center_id: $control_center_id){control_center}
  dataSource: control_center_actual(type: $type, record_type:"weekly", data_set: "level_1",control_center_id: $control_center_id, control_element: "A2", order: ["t_run"]) {
    id
    control_center_id
    t_run
    type
