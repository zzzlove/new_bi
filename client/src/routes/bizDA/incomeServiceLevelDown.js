...
function RenderComponent({dispatch, bizDataAnalysis, data }) {
  ...
  const { xType, timeSelectMode, time_started_at, time_finished_at, format_started_at, format_finished_at, incomeServiceLevelDownVisible, titleLevelDownService} = bizDataAnalysis; 
  
 
  if (!incomeServiceLevelDownVisible) {
    return false;
  }
  
  const { sales_yd_amount_service_gb_range, sales_yd_amount_service_range_gb_tradername, loading, error } = data; 
  ...
  if (error) {
    ...
    const ChartDefs = createG2((chart, data) => {
     ...
     if (sales_yd_amount_service_gb_range.length==0) { 
        return (<h3><Icon type="frown-o"/>暂无对应数据 </h3>)
     }

     const SalesRank = () =>   <div className={styles.salesRank}>
          <Tabs size="small">
            <Tabs.TabPane tab="发货客户" key="service">
              <ul className={styles.rankingList}>
                {
                  sales_yd_amount_service_range_gb_tradername.map((item, i) => (
                    <li key={item.c}>
    ...
    const HideServiceDetails = (<div>
        <Button type="primary" onClick={()=>dispatch({type: 'bizDataAnalysis/setDetails',  payload: {incomeServiceLevelDownVisible: false}, })}><Icon type="close" style={{fontSize: '14px'}}/>关闭窗口</Button>
      </div>)
     ...
     return (
        ...
        >
          <div className={styles.salesCard}>
            <Tabs size="large" tabBarStyle={{ marginBottom: 24 }} tabBarExtraContent={HideServiceDetails} >
              <Tabs.TabPane tab={<span>{titleLevelDownService}产值分解</span>} key="trader_name">
                <Row>
                  <Col xl={16} lg={12} md={12} sm={24} xs={24}>
                    <div className={styles.salesBar}>
                      <ChartDefs
                        data={sales_yd_amount_service_gb_range}
                        width={900}
                        height={500}
...
const RenderComponentWithMutations = compose(
  graphql(IncomeServiceLevelDownQuery, { name: 'data', options: (props) => {return { variables: { ops_id: props.user.ops_id, ...props.bizDataAnalysis.incomeDaFetchProp } }} }),
)(RenderComponent);
