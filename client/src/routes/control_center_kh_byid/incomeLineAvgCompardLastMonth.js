...
const Chart1Defs = createG2((chart, data) => {
    ...
    amount_avg: {alias: "收入日平均(单位: 万元)", formatter: function (value) {
    ...
    chart.axis('amount_avg', {labelOffset: 20, line: false });
    chart.axis('t_time', {title: false });

    chart.legend({position: 'bottom'});
    chart.tooltip({ title: null });

    chart.line().position('t_time*amount_avg').color('cat_1st').shape('spline').size(3).opacity(0.8).tooltip('t_time*cat_1st*amount_avg');
//    chart.line().position('t_time*amount_acc').color('cat_2nd').shape('spline').opacity(0.8).tooltip('t_time*cat_2nd*amount_avg');
...
query MyQuery($control_center_id: [Int]) {
  dataSource: chart_data(chart_no: "K02L0A", control_center_id: $control_center_id) {
    id
    t_time
    cat_1st
    cat_2nd
